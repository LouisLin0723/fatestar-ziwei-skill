...
- 渠道标记：所有 CLI 请求发送 `X-FateStar-Client: skill/2.3.0`。
...
的运限目标。不传目标则默认：当前农历年月日 + 出生时辰。

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| --target-year | int | 否 | 流年目标年（默认当前农历年） |
| --target-month | int | 否 | 流月目标农历月 1-12（默认当前农历月） |
| --target-day | int | 否 | 流日目标农历日 1-30（默认当前农历日） |
...
- 所有网络请求标记 `skill/2.3.0`，用于后台区分 API / MCP / Skill。
